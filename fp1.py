...
#ptn = 0
#for P in RR:
#	print('draw pt'+str(ptn)+' {'+str(P.first)+','+str(P.second)+','+str(P.third)+'}')
#	ptn = ptn+1
...
#clustering analysis
import itertools
clf1 = KMeans(n_clusters = 7)
s1 = clf1.fit(RMatrix)
clf2 = KMeans(n_clusters = 6)
s2 = clf2.fit(BMatrix)
#find the nearest point of the cluster center
Nearest = 0
RVMatrix = np.zeros((7, 3))
BVMatrix = np.zeros((6, 3))
RNMatrix = np.zeros((7, 3))
BNMatrix = np.zeros((6, 3))
count = 0
hang = 0
...
#for row in clf2.cluster_centers_:
#	print('draw ptb'+str(row[0])+' {'+str(row[0])+','+str(row[1])+','+str(row[2])+'}')
scipy.io.savemat('fp1.mat', {'RPT':RNMatrix, 'BPT':BNMatrix, 'RVT':RVMatrix, 'BVT':BVMatrix})
