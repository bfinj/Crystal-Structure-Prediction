...
T = [0, 0, 0]
OutFile = open('fpoutput', 'w')
for i in range(12):
	for j in range(6):
		matfn = 'fp'+str(i+1)+str(j+1)+'.mat'
...
#print NewRT4
				...
				if max(abs(a),abs(b),abs(c))<15 and max(abs(a),abs(b),abs(c))>13 and min(abs(a),abs(b),abs(c))>3.9 and min(abs(a),abs(b),abs(c))<5.9 and abs(a)+abs(b)+abs(c)-max(abs(a),abs(b),abs(c))-min(abs(a),abs(b),abs(c))>12.6 and abs(a)+abs(b)+abs(c)-max(abs(a),abs(b),abs(c))-min(abs(a),abs(b),abs(c))<14.6:
					OutFile.write(matfn)
