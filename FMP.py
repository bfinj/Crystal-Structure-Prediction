...
data3 = sio.loadmat('fp1.mat')
#RT = data2['RPT']
RT1 = np.transpose(data2['RPT'])
RedTable1=[[0,0],[0,0],[0,0]]
RedTable2=[[0,0],[0,0],[0,0]]
#BT = data2['BPT']
RT2 = np.transpose(data3['RPT'])
T = [0, 0, 0]
OutFile = open('foutput', 'w')
for i in range(12):
	for j in range(7):
		matfn = 'f'+str(i+1)+str(j+1)+'.mat'
		data = sio.loadmat(matfn)
		Rint = data['Results']
		for m in range(12):
			for k in range(7):
				for n in range(3):
					RedTable1[n][0]=RT1[n][i]
					RedTable2[n][0]=RT2[n][j]
					RedTable1[n][1]=RT1[n][m]
					RedTable2[n][1]=RT2[n][k]
				NewRT1 = np.dot(Rint, RedTable1)
				NewRT2 = np.dot(M1, NewRT1)
				NewRT3 = np.dot(M2, NewRT1)
				NewRT4 = np.dot(M3, NewRT1)
				NewBT1 = np.dot(Rint, RedTable2)
